...
static int devfs_mkdir(dentry_t *dir, dentry_t *dntr, int mode)
{
    ...
    if ((dir->d_flags & T_IFDIR) == 0 || (dntr->d_flags & T_IFDIR) == 0)
        return -EINVAL;

    if (hm_get(dir->d_children, dntr->d_name) != NULL)
        return -EEXIST;
...
static int devfs_mknod(dentry_t *dir, dentry_t *dntr, int mode, dev_t rdev)
{
    ...
    (void)dir, (void)dntr, (void)mode, (void)rdev;
    return -ENOSYS;
}
...
static inode_t *devfs_inode_alloc(superblock_t *sb)
{
    ...
    ino->i_iops->rename   = NULL; ino->i_iops->follow_link = NULL; ino->i_fops->write      = NULL;
...
int devfs_register_cdev(cdev_t *dev, char *name)
{
    ...
    if (retpath->p_status & LOOKUP_STAT_EEXISTS) {
        kdebug("%s already exists!", path);
        ret = -EEXIST;
        goto end;
    }

    if ((ino = devfs_inode_alloc(retpath->p_dentry->d_inode->i_sb)) == NULL) {
        ret = -errno;
        goto end;
    }

    if ((dntr = dentry_alloc_ino(retpath->p_dentry, name, ino, T_IFCHR)) == NULL) {
