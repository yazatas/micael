...
void vmm_init(void)
{
    ...
    kdebug("kernel page directory: 0x%x", vmm_v_to_p(&kpdir));
    kdebug("kernel page table: 0x%x", vmm_v_to_p(&kpgtab));

    kpdir[KSTART_HEAP] = vmm_kalloc_frame() | P_PRESENT  | P_READWRITE;
    kpdir[KSTART]      = ((uint32_t)vmm_v_to_p(&kpgtab)) | P_PRESENT | P_READWRITE;
    kpdir[1023]        = ((uint32_t)vmm_v_to_p(&kpdir)) | P_PRESENT;

    vmm_heap_init();

    kdebug("changing page directory...");
...
/* assumes that virtaddr points to the beginning of page directory */
void vmm_list_pde(void)
{
    kdebug("mapped PDEs:");
    uint32_t *v = (uint32_t*)0xfffff000;
    for (size_t i = 0; i < 1024; ++i) {
        if (v[i] & P_PRESENT)
...
void vmm_list_pte(uint32_t pdi)
{
    ...
    kdebug("mapped bytes: %uB %uMB", nbytes, nbytes / 1000);
    kdebug("mapped table entries: %u", nbytes / 0x1000);
}
